...
#   ${body}=   create dictionary   to json    ${EMPTY}
    ${header}=  create dictionary   Authorization=${TOKEN}     Content-Type=application/json
     ${response}=   get request    post_01   /mah/api/integrations_conf/get-apikey     headers=${header}
...
##validations
   ${status_code}=    convert to string    ${response.status_code}
       ...
       should contain   ${res_body}     Data fetch successfully
...
#   ${body}=   create dictionary   to json    ${EMPTY}
    ${header}=  create dictionary   Authorization=${TOKEN}     Content-Type=application/json
     ${response}=  post request    post_01   /mah/api/integrations_conf/get-apikey     headers=${header}
    Log To Console    ${response.status_code}
    Log To Console    ${response.content}
##validations
   ${status_code}=    convert to string    ${response.status_code}
