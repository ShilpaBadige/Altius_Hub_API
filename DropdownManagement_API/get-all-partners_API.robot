...
*** Test Cases ***
case1:get-all-partners API with positive flow
     create session    post_01   ${Altius_Hub_Base_Url}     verify=${TRUE}
#   ${body}=   create dictionary   to json    ${EMPTY}
    ${header}=  create dictionary    Content-Type=application/json
     ${response}=  get request    post_01   /mah/api/principle_database/get-all-partners    headers=${header}
...
case2:get-all-partners API (with invalid method name)
      create session    post_01   ${Altius_Hub_Base_Url}     verify=${TRUE}
#   ${body}=   create dictionary   to json    ${EMPTY}
    ${header}=  create dictionary      Content-Type=application/json
     ${response}=  post request    post_01   /mah/api/principle_database/get-all-partners     headers=${header}
    Log To Console    ${response.status_code}
    Log To Console    ${response.content}
##validations
   ${status_code}=    convert to string    ${response.status_code}
